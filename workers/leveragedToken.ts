...
async function snapshot(vaultContractAddress: string, leveragedTokenContractAddress: string, collateralDecimals: number, debtDecimals: number, provider: ethers.providers.JsonRpcProvider, connection: Connection) {
    // Initialize the contract
    const riseTokenVaultContract = new ethers.Contract(vaultContractAddress, VaultABI, provider);
    const leveragedTokenContract = new ethers.Contract(leveragedTokenContractAddress, ERC20ABI, provider);

    // Get collateral per leveraged token
    const collateralPerLeveragedTokenBN: BigNumber = await riseTokenVaultContract.getCollateralPerRiseToken(leveragedTokenContractAddress);
    const collateralPerLeveragedToken = parseFloat(ethers.utils.formatUnits(collateralPerLeveragedTokenBN, collateralDecimals));
    console.log("DEBUG: collateralPerLeveragedToken", collateralPerLeveragedToken);

    // Get debt per leveraged token
    const debtPerLeveragedTokenBN: BigNumber = await riseTokenVaultContract.getDebtPerRiseToken(leveragedTokenContractAddress);
    const debtPerLeveragedToken = parseFloat(ethers.utils.formatUnits(debtPerLeveragedTokenBN, debtDecimals));
    console.log("DEBUG: debtPerLeveragedToken", debtPerLeveragedToken);

    // Get leverage ratio in ether
    const leverageRatioBN: BigNumber = await riseTokenVaultContract.getLeverageRatioInEther(leveragedTokenContractAddress);
