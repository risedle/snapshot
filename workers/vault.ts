...
async function snapshot(
    ...
    // Get max capacity
    const maxTotalDeposit: BigNumber =
        await riseTokenVaultContract.maxTotalDeposit();
    const maxTotalDepositFloat = parseFloat(
        ethers.utils.formatUnits(maxTotalDeposit, 6)
    );
